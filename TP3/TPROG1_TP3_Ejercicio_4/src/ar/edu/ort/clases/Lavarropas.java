package ar.edu.ort.clases;

public class Lavarropas extends  Electrodomestico implements Imprimible{
    private int cargaMax;
    private Tipo tipo;



    public Lavarropas(String marca, String modelo, String nroSerie, int voltaje, boolean estado, double precio, int cargaMax, Tipo tipo) {
        super(marca, modelo, nroSerie, voltaje, estado, precio);
        this.cargaMax = cargaMax;
        this.tipo = tipo;
    }

    @Override
    public String toString() {
        return getClass().getSimpleName() + " " + super.getMarca() + " " + tipo + ", carga maxima " + cargaMax + ", modelo " +
                super.getModelo() + " => Precio: " + super.getPrecio();
    }

    @Override
    public void imprimir() {
        System.out.printf("%s %s %s, carga maxima %d, modelo %s => Precio: %.2f\n", getClass().getSimpleName(), super.getMarca(),this.tipo,this.cargaMax,super.getModelo(),super.getPrecio());
    }
}
